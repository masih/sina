...
public class normal {
    ...
    public static double cumulative(double x, double mu, double sigma) {
        ...
        /*!* #ifdef IEEE_754 /*4!*/
        if (Double.isNaN(x) || Double.isNaN(mu) || Double.isNaN(sigma)) return x + mu + sigma;
        /*!* #endif /*4!*/
        ...
        if (y <= 0.66291) {
            ...
            result = (Constants.SQRT_2_PI - result) / y;
    ...
    public static double density(double x, double mu, double sigma) {
        ...
        x = (x - mu) / sigma;
        return Constants.SQRT_2_PI * java.lang.Math.exp(-0.5 * x * x) / sigma;
    }
    ...
    public static double quantile(double p, double mu, double sigma) {
        ...
        /*!* #endif /*4!*/
        if (p < 0.0 || p > 1.0) { throw new java.lang.ArithmeticException("Math Error: DOMAIN");
        //    	return Double.NaN;
        }
    ...
    public static double random(double mu, double sigma, uniform PRNG) {
        ...
        sigma < 0.0) {
            ...
            return mu;
        else return mu + sigma * random(PRNG);
    }
    ...
    public static double random(uniform PRNG) {
        ...
        if (u1 >= 0.958720824790463) {
            region3: while (true) {
                ...
                if (Math.max(u2, u3) <= 0.755591531667601) return (u2 < u3) ? t : -t;
                /*!* 	if(0.034240503750111*fabs(u2-u3) <= g(t)) *!*/
                if (0.034240503750111 * java.lang.Math.abs(u2 - u3) <= g(t)) return (u2 < u3) ? t : -t;
                // continue region3;
            }
        }
        ...
        if (u1 >= 0.911312780288703) {
            region2: {
                ...
                if (Math.max(u2, u3) <= 0.872834976671790) return (u2 < u3) ? t : -t;
                /*!* 	if( 0.049264496373128*fabs(u2-u3)<=g(t) ) *!*/
                if (0.049264496373128 * java.lang.Math.abs(u2 - u3) <= g(t)) return (u2 < u3) ? t : -t;
                // continue region2;
            }
        }
        ...
        region1: while (true) {
            ...
            if (Math.max(u2, u3) <= 0.805577924423817) return (u2 < u3) ? t : -t;
            // continue region1;
        }
    }
    ...
    public static double random_AhrensDieter(uniform PRNG) {
        ...
        final double a[] = {0.0000000, 0.03917609, 0.07841241, 0.1177699, 0.1573107, 0.19709910, 0.23720210, 0.2776904, 0.3186394, 0.36012990, 0.40225010, 0.4450965, 0.4887764, 0.53340970, 0.57913220, 0.6260990, 0.6744898, 0.72451440, 0.77642180, 0.8305109, 0.8871466, 0.94678180, 1.00999000,
                        1.0775160, 1.1503490, 1.22985900, 1.31801100, 1.4177970, 1.5341210, 1.67594000, 1.86273200, 2.1538750};

        final double d[] = {0.0000000, 0.0000000, 0.0000000, 0.0000000, 0.0000000, 0.2636843, 0.2425085, 0.2255674, 0.2116342, 0.1999243, 0.1899108, 0.1812252, 0.1736014, 0.1668419, 0.1607967, 0.1553497, 0.1504094, 0.1459026, 0.1417700, 0.1379632, 0.1344418, 0.1311722, 0.1281260, 0.1252791,
                        0.1226109, 0.1201036, 0.1177417, 0.1155119, 0.1134023, 0.1114027, 0.1095039};

        final double t[] = {7.673828e-4, 0.002306870, 0.003860618, 0.005438454, 0.007050699, 0.008708396, 0.010423570, 0.012209530, 0.014081250, 0.016055790, 0.018152900, 0.020395730, 0.022811770, 0.025434070, 0.028302960, 0.031468220, 0.034992330, 0.038954830, 0.043458780, 0.048640350,
                        0.054683340, 0.061842220, 0.070479830, 0.081131950, 0.094624440, 0.112300100, 0.136498000, 0.171688600, 0.227624100, 0.330498000, 0.584703100};

        final double h[] = {0.03920617, 0.03932705, 0.03950999, 0.03975703, 0.04007093, 0.04045533, 0.04091481, 0.04145507, 0.04208311, 0.04280748, 0.04363863, 0.04458932, 0.04567523, 0.04691571, 0.04833487, 0.04996298, 0.05183859, 0.05401138, 0.05654656, 0.05953130, 0.06308489, 0.06737503,
                        0.07264544, 0.07926471, 0.08781922, 0.09930398, 0.11555990, 0.14043440, 0.18361420, 0.27900160, 0.70104740};
