...
public class noncentral_chisquare {
    ...
    public static double density(double x, double df, double lambda) {
        ...
        /*!* #ifdef IEEE_754 /*4!*/
        if (Double.isNaN(x) || Double.isNaN(df) || Double.isNaN(lambda)) return x + df + lambda;
        /*!* #endif /*4!*/

        if (lambda < 0 || df <= 0) { throw new java.lang.ArithmeticException("Math Error: DOMAIN"); }

        /*!* #ifdef IEEE_754 /*4!*/
        if (Double.isInfinite(df) || Double.isInfinite(lambda)) { throw new java.lang.ArithmeticException("Math Error: DOMAIN");
        //    		return Double.NaN;
        }
        /*!* #endif /*4!*/
    ...
    public static double cumulative(double x, double f, double theta) {
        ...
        /*!* #ifdef IEEE_754 /*4!*/
        if (Double.isNaN(x) || Double.isNaN(f) || Double.isNaN(theta)) return x + f + theta;
        if (Double.isInfinite(f) || Double.isInfinite(theta)) { throw new java.lang.ArithmeticException("Math Error: DOMAIN");
        //		return Double.NaN;
        }
        /*!* #endif /*4!*/

        if (f < zero && theta < zero) { throw new java.lang.ArithmeticException("Math Error: DOMAIN");
        //	    return Double.NaN;
        }
        if (x <= zero) return 0;
        /*!* #ifdef IEEE_754 /*4!*/
        ...
        L_End: for (;;) {
            ...
            if (!(f + twon - x > zero)) {
                ...
                for (;;) {
                    ...
                    /*!* #endif /*4!*/
                    if (bound <= errmax || n > itrmax) break L_End;
                    /* evaluate the next term of the */
                    /* expansion and then the partial sum */
                    u *= lam / n;
                    v += u;
        ...
        if (bound > errmax) throw new java.lang.ArithmeticException("Math Error: PRECISION");
        /*!* #ifdef DEBUG_pnch /*4!*/
    ...
    public static double quantile(double p, double n, double lambda) {
        ...
        /*!* #ifdef IEEE_754 /*4!*/
        if (Double.isNaN(p) || Double.isNaN(n) || Double.isNaN(lambda)) return p + n + lambda;
        if (Double.isInfinite(n)) { throw new java.lang.ArithmeticException("Math Error: DOMAIN");
        //    		return Double.NaN;
        }
        /*!* #endif /*4!*/
        /*!* 	n = floor(n + 0.5); *!*/
        n = java.lang.Math.floor(n + 0.5);
        if (p < 0 || p >= 1 || n < 1 || lambda < 0) { throw new java.lang.ArithmeticException("Math Error: DOMAIN");
        //    		return Double.NaN;
        }
        if (p == 0) return 0;
