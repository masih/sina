...
public class logistic {
    ...
    public static double quantile(double x, double location, double scale) {
        ...
        if (Double.isNaN(x) || Double.isNaN(location) || Double.isNaN(scale)) return x + location + scale;
        /*!* #endif /*4!*/
        if (scale <= 0.0 || x < 0 || x > 1) { throw new java.lang.ArithmeticException("Math Error: DOMAIN");
        //    		return Double.NaN;
        }
    ...
    public static double random(double location, double scale, uniform PRNG) {
        ...
        double u;
        /* #ifndef IEEE_754 */
        if (Double.isInfinite(location) || Double.isInfinite(scale)) { throw new java.lang.ArithmeticException("Math Error: DOMAIN");
        //    		return Double.NaN;
        }
        /* #endif */
        u = PRNG.random();
